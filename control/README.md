...
3. Read only the new file(s), act or plan the change, advance the cursor, and record the ack
   ...
   - **appended to the directive file itself** — add at the bottom:
     ...
     block is the only thing an agent may ever add to an existing directive. This makes every
     directive a self-contained, durable receipt: instruction + proof-of-read + action, in
     git. A directive without an ack block is by definition not yet processed — the issuer can
     see at a glance what has and hasn't been picked up.
4. **Check cadence:** at minimum — start of every operating-loop step (not just every unit),
...
5. Never re-read the standing docs on a poll. Re-read a standing doc only when a directive
...
4. any directives above the cursor
